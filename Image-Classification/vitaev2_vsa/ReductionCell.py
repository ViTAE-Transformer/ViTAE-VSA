...
from .vsa import VSAWindowAttention, Mlp
...
class ReductionCell(nn.Module):
    def __init__(self, img_size=224, in_chans=3, embed_dims=64, wide_pcm=True, token_dims=64, downsample_ratios=4, kernel_size=7,
                 num_heads=1, dilations=[1,2,3,4], share_weights=False, op='cat', tokens_type='VSA', group=1,
                 relative_pos=False, cpe=False, drop=0., attn_drop=0., drop_path=0., mlp_ratio=1.0, window_size=7):
        super().__init__()
        ...
        if tokens_type == 'performer':
            ...
            self.attn = VSAWindowAttention(
                ...
                attn_drop=attn_drop, proj_drop=drop, img_size=(self.img_size//self.downsample_ratios, self.img_size//self.downsample_ratios))
    ...
    def forward(self, x):
        ...
        if self.tokens_type == 'VSA':
            ...
            assert N == H*W
            if self.cpe:
                PRM_x = PRM_x + self.pos(PRM_x.permute(0, 2, 1).reshape(B, C, H, W)).reshape(B, C, N).permute(0, 2, 1)
            x = self.norm1(PRM_x)
            x = x.view(B, H, W, C)

...
class BasicLayer(nn.Module):
    def __init__(self, img_size=224, in_chans=3, embed_dims=64, wide_pcm=True, token_dims=64, downsample_ratios=4, kernel_size=7, RC_heads=1, NC_heads=6, dilations=[1, 2, 3, 4],
                ...
                relative_pos=False, cpe=False, num_deform=None):
        super().__init__()
        self.img_size = img_size
        ...
        if RC_tokens_type == 'stem':
            ...
            self.RC = ReductionCell(img_size, in_chans, embed_dims, wide_pcm, token_dims, downsample_ratios, kernel_size,
                            RC_heads, dilations, op=RC_op, tokens_type=RC_tokens_type, group=RC_group, window_size=window_size, relative_pos=relative_pos, cpe=cpe, num_deform=num_deform)
        else:
            self.RC = nn.Identity()
        self.NC = nn.ModuleList([
            NormalCell(token_dims, NC_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale, drop=drop, attn_drop=attn_drop,
                       ...
                       img_size=img_size // downsample_ratios, window_size=window_size, shift_size=0, relative_pos=relative_pos, cpe=cpe, num_deform=num_deform)
...
class ViTAE_VSA_basic(nn.Module):
    def __init__(self, img_size=224, in_chans=3, stages=4, embed_dims=64, wide_pcm=True, token_dims=64, downsample_ratios=[4, 2, 2, 2], kernel_size=[7, 3, 3, 3], 
                RC_heads=[1, 1, 1, 1], NC_heads=4, dilations=[[1, 2, 3, 4], [1, 2, 3], [1, 2], [1, 2]],
                RC_op='cat', RC_tokens_type=['VSA', 'VSA', 'VSA', 'VSA'], NC_tokens_type='VSA',
                ...
                window_size=7, relative_pos=False, cpe=False, num_deform=None):
        super().__init__()
        ...
        self.wide_pcm = repeatOrNot(wide_pcm, stages)
        ...
        for i in range(stages):
            ...
            Layers.append(
                BasicLayer(img_size, in_chans, self.embed_dims[i], self.wide_pcm[i], self.tokens_dims[i], self.downsample_ratios[i],
                ...
                norm_layer=self.norm_layer[i], window_size=window_size, relative_pos=relative_pos, cpe=cpe, num_deform=num_deform)
            )

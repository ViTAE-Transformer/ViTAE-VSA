...
class NormalCell(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                ...
                shift_size=0, window_size=0, img_size=224, relative_pos=False, cpe=False, num_deform=None):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.tokens_type == 'window':
            ...
            assert n == self.img_size * self.img_size, "input feature has wrong size"
            if self.cpe:
                x = x + self.pos(x.permute(0, 2, 1).reshape(b, c, H, W)).reshape(b, c, n).permute(0, 2, 1)
            x = self.norm1(x)
            x = x.view(b, H, W, c)
            x = x.permute(0, 3, 1, 2)
            x = self.attn(x)
